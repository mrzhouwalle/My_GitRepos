 ...
 * CONSTANTS (常量定义)
 */
static bool simple_link_ntf_enable = false;
static uint8_t con_index = 0;
...
const gatt_attribute_t simple_profile_att_table[SP_IDX_NB] =
{
    ...
    [SP_IDX_SERVICE]                        =   {
                                                    ...
                                                    GATT_PROP_AUTHEN_READ,                                             /* Permissions */
        ...
        [SP_IDX_CHAR1_VALUE]                =   {
                                                    ...
                                                    GATT_PROP_AUTHEN_READ | GATT_PROP_WRITE,                           /* Permissions */
...
static void sp_gatt_write_cb(uint8_t *write_buf, uint16_t len, uint16_t att_idx)
{
	...
	if (uuid == GATT_CLIENT_CHAR_CFG_UUID)
    {
        ...
        if (att_idx == SP_IDX_CHAR4_CFG)
        {
			...
			simple_link_ntf_enable = true;
			simple_gatt_report_notify(con_index, 0, 0);
        }
    }
 ...
 * @return  None.
 */
void simple_gatt_report_notify(uint8_t conidx, uint8_t *p_data, uint16_t len)
{
	uint8_t data[100] = {
 ...
 * @return  None.
 */
void simple_gatt_notify_send(uint8_t *p_data, uint16_t len)
{
    if (simple_link_ntf_enable)
    {
        gatt_ntf_t ntf;
        ntf.conidx = con_index;
        ntf.svc_id = sp_svc_id;
        ntf.att_idx = SP_IDX_CHAR4_VALUE;
        ntf.data_len = len;
        ntf.p_data = p_data;
